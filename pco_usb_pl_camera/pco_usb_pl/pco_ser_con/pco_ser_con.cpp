...
//following code should work with pco.edge
//we set signal at output 3 to both options and
//if available set additional timing parameter
    if(numSignals>=3)
    {
     ...
     {
      Parameter=HW_IO_SIGNAL_TIMING_EXPOSURE_RS_FIRSTLINE;
      printf("call  PCO_SetHWIOSignalTiming(3,0,%d)\n",Parameter);
     ...
     if(signaldesc3.wSignalDefinitions&HW_IO_SIGNAL_TIMING_1_AVAILABLE)
     {
      Parameter=HW_IO_SIGNAL_TIMING_EXPOSURE_RS_FIRSTLINE;
      printf("call  PCO_SetHWIOSignalTiming(3,0,%d)\n",Parameter);
...
int main(int argc, char *argv[])
{
  ...
  for(int i=argc;i>1;)
  {
   ...
   if((a=strstr(argv[i],"-l")))
   {
    ...
    mylog=new CPco_Log("pco_ser_con.log");
    if(x>0)
    {
     x|=0x03;
     mylog->set_logbits(x);
