...
int switch_camera(CPco_com_usb* camera,int mode,int cam_num);
...
int main(int argc, char* argv[])
{
  ...
  if(mode==111)
  {
    printf("usage: %s options\n"
           "options: \n"
           "-b[0...7]  actual device number\n"
           "-RS        change to RollingShutter\n"
           "-GS        change to GlobalShutter\n"
           "-GR        change to GlobalReset\n"
//           "-f         force setting\n"
           "-h,-?,? this message\n",argv[0]);
    return 0;
  }
  ...
  if(err!=PCO_NOERROR)
   printf("Get camera information Error 0x%x\n",err);
  else
  {
  ...
  if((camtype!=CAMERATYPE_PCO_EDGE_USB3)||(err!=PCO_NOERROR))
  {
...
//  printf("Camera setup_id is %d setup_flags %d,%d,%d,%d\n",setup_id,setup_flag[0],setup_flag[1],setup_flag[2],setup_flag[3]);
  ...
  if(mode==0)
  {
   if(setup_flag[0]==PCO_EDGE_SETUP_ROLLING_SHUTTER)
   {
    ...
    if((description.dwGeneralCaps1&GENERALCAPS1_GLOBAL_RESET_MODE)==GENERALCAPS1_GLOBAL_RESET_MODE)
   ...
   {
    printf("Camera in GlobalReset mode\n");
    printf("'pco_switch_edge -RS' to set camera to RollingShutter mode\n");
    if((description.dwGeneralCaps1&GENERALCAPS1_NO_GLOBAL_SHUTTER)!=GENERALCAPS1_NO_GLOBAL_SHUTTER)
     printf("'pco_switch_edge -GS' to set camera to GlobalShutter mode\n");
   }
  ...
  else if(mode==1)
  {
   if(setup_flag[0]==PCO_EDGE_SETUP_ROLLING_SHUTTER)
   {
    ...
    if((description.dwGeneralCaps1&GENERALCAPS1_NO_GLOBAL_SHUTTER)==0)//GENERALCAPS1_NO_GLOBAL_SHUTTER)
     printf("'pco_switch_edge -GS' to set camera to GlobalShutter mode\n");
    if((description.dwGeneralCaps1&GENERALCAPS1_GLOBAL_RESET_MODE)==GENERALCAPS1_GLOBAL_RESET_MODE)
     printf("'pco_switch_edge -GR' to set camera to GlobalReset mode\n");
   }
  ...
  else if(mode==3)
  {
   ...
   if(force==1)
   {
    printf("Set camera to GlobalReset\n");
    err=switch_camera(cam,mode,cam_num);
    if(err==PCO_NOERROR)
     printf("done successfully\n");
   }
...
int switch_camera(CPco_com_usb* cam,int mode,int cam_num)
{
...
//wait before reboot
  ...
  err=cam->PCO_GetCameraSetup(&setup_id,rsetup_flag,&length);
  if(err!=PCO_NOERROR)
  {
...
//  printf("getCameraSetup setup_id is %d setup_flags %d,%d,%d,%d\n",setup_id,rsetup_flag[0],rsetup_flag[1],rsetup_flag[2],rsetup_flag[3]);
  if(rsetup_flag[0]!=setup_flag[0])
  {
   printf("switch camera failed %d!=%d\n",rsetup_flag[0],setup_flag[0]);
   err=PCO_ERROR_VALIDATION|PCO_ERROR_APPLICATION|PCO_ERROR_SDKAPPLICATION;
  }
